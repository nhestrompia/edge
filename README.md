...
- Asset symbols from Hyperliquid are resolved to official marks from `https://app.hyperliquid.xyz/coins/{symbol}.svg`; the existing authored marks remain as the loading and failure fallback. Neither the Binance ticker payload nor the Hyperliquid position payload includes an icon URL.
